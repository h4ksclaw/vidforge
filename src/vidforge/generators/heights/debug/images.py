...
class ImagesDebug(DebugScript):
    ...
    def run(self, wiki: str, page: str, max_inspect: int = 10) -> str | None:
        ...
        passed_filter: list[tuple[float, str, str]] = []
        ...
        for score, fname, url in passed_filter[:max_inspect]:
            ...
            img.save(orig_path)
            orig_url = self.upload_asset(orig_path)
            if orig_url:
            ...
            section.add_status(
                f"content_ratio: {cr:.3f} {'✅' if cr_ok else '❌ too wide'}", ok=cr_ok
            )
            section.add_status(
                f"height_fill: {hf:.3f} {'✅' if hf_ok else '❌ too short'}", ok=hf_ok
            )
            ...
            if processed:
                ...
                proc_url = self.upload_asset(proc_path)
                if proc_url:
                ...
                if cr2 <= MAX_CONTENT_RATIO and hf2 >= MIN_HEIGHT_FILL:
                    pass_quality += 1
...
def main() -> None:
    ...
    ImagesDebug()(wiki=wiki, page=page, max_inspect=max_inspect)
